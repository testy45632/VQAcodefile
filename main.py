...
def process_text(text):
    ...
    contractions = {
        "dont": "dont, isnt: isnt", "arent": "arent, wont: wont",
        "cant": "cant, wouldnt: wouldnt", "couldnt": "couldnt
    }
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1):
        ...
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernelsize=3, stride=1, padding=1)
        self.bn2 = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace=True)
        self.shortcut = nn.Sequential()
        if stride != 1 or in_channels != out_channels:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_channels, out_channels, kernelsize=1, stride=stride),
...
class BottleneckBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernelsize=1, stride=1)
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernelsize=3, stride=stride, padding=1)
        self.bn2 = nn.BatchNorm2d(out_channels)
        self.conv3 = nn.Conv2d(in_channels, out_channels * self.expansion, kernelsize=1, stride=1)
        self.bn3 = nn.BatchNorm2d(out_channels * self.expansion)
        self.relu = nn.ReLU(inplace=True)
        self.shortcut = nn.Sequential()
        if stride != 1 or in_channels != out_channels * self.expansion:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_channels, out_channels * self.expansion, kernelsize=1, stride=stride),
...
class ResNet(nn.Module):
    def __init__(self, block, layers):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernelsize=3, stride=2, padding=1)
